...
#if defined(__OBJC__)
/// 绿幕类型
typedef SWIFT_ENUM(NSInteger, GreenScreenType, closed) {
/// 绿色
  GreenScreenTypeGreenScreen_green = 0,
/// 蓝色
  GreenScreenTypeGreenScreen_blue = 1,
};
...
/// 常规滤镜，参数范围：0.0 - 100.0
@property (nonatomic) float filter_delta;
/// 常规滤镜名称（暂时）
@property (nonatomic, copy) NSString * _Nonnull filter_name;
@property (nonatomic, strong) UIImage * _Nullable filter_image;
/// 绿幕抠图开关
@property (nonatomic) BOOL isOpenGreenScreen;
/// 绿幕类型
@property (nonatomic) enum GreenScreenType greenScreen_type;
/// 绿幕抠图，参数范围：0.0 - 100.0
@property (nonatomic) float greenScreen_delta;
/// 填充绿幕的背景图（建议先按CVPixelBuffer的比例裁剪好尺寸）
@property (nonatomic, strong) UIImage * _Nonnull greenScreen_background;

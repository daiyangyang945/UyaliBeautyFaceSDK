...
class BeautyImageController: UIViewController,FaceReshapeDelegate,FaceBeautyDelegate,FaceMakeupDelegate {
    ...
    private var openGLView: GLKPreviewView!
    ...
    override func viewDidLoad() {
        ...
        openGLView = GLKPreviewView(frame: CGRect(x: (view.bounds.width-imageWidth)/2.0, y: (view.bounds.height-imageHeight)/2.0, width: imageWidth, height: imageHeight))
    ...
    private func startToProcessImage() {
        ...
        CVPixelBufferLockBaseAddress(originPixelBuffer, CVPixelBufferLockFlags(rawValue: 0))
        timer = DispatchSource.makeTimerSource(queue: DispatchQueue.global())
        timer.schedule(deadline: .now(), repeating: .microseconds(100))
        timer.setEventHandler(handler: {[weak self] in
            guard let self = self else { return }
            context.render(self.ciImage!, to: self.pixelBuffer!)
    ...
    private func stopTimer() {
        CVPixelBufferUnlockBaseAddress(originPixelBuffer, CVPixelBufferLockFlags(rawValue: 0))
        if timer != nil {
            timer.cancel()
            timer = nil
        }
